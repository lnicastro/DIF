/*
  ...
  HTM/HELAPix pixels indexed MySQL tables.
   ...
   Notes:
	A simplified version derived from pix_myXmatch.cc which requires a unique "_id"
	to be present in both catalogues!
  ...
  1. full catalogue match saving matched and unmatched objects and using neighbors pixels
     of depth 14 (about 15 arcsec) to those of depth 6. Specify in/out DBs with table names.

    pix_myXmatch_id -x DBin1.ascc25 DBin2.tycho2 -t DBout.xout_tab -D 6 14 -qA

  2. give pixel IDs range
...
*/
	...
	qry_str = qry_str_i + tabname +" SELECT i."+ t.id_coln1 +" AS "+ t.id_coln1 +",r."+ t.id_coln1 +" AS ref_"+ t.id_coln1 +
	 ",i."+ t.rf_coln1 +",r."+ t.rf_coln2 +" AS ref"+ t.rf_coln2 +
	 " FROM "+ db.my_db1 +dt+ db.cat1 +" AS i,"+ db.my_db2 +dt+ db.cat2 +" AS r LIMIT 0";

      } else if (i == 1) {

// Unmatched objects table
	tabname = t.otab.nx;
	qry_str = qry_str_i + tabname +" SELECT "+ t.id_coln1 +co+ t.rf_coln1 +
		" FROM "+ db.my_db1 +dt+ db.cat1 +" LIMIT 0";
...
// HTM IDs column name
  ...
  cout <<"---> RefCat ("<< db.cat2 <<") fields: "<< t.id_coln1 <<", "<< t.ra_coln2 <<", "<< t.de_coln2;
	...
	cout <<"; added reference field: ref"<< t.rf_coln2;
   cout << endl << endl;
...
// Create the temporary table for main pixel+border pixels selection storage only once
  ...
  qry_str = "CREATE TEMPORARY TABLE "+ tmp_tab +
		" SELECT "+ ra_fld1 +" as RAdeg, "+ de_fld1 +" as DEdeg, "+ t.id_coln1 +co+ t.rf_coln1 +
		" FROM "+ db.my_db1 +dt+ db.cat1 +" LIMIT 0";
...
// To maintain the same nr of cols use dummy input id
  ...
  difqry_ini1 = " SELECT DISTINCT "+ ra_fld1 +co+ de_fld1 +co+ t.id_coln1 +co+ t.rf_coln1 +" FROM ";
