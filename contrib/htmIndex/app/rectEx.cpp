...
printRanges(size_t nRanges, ValueVector &ranges) {
  for(size_t i = 0; i < nRanges; i++) {
    ...
    PRINTID(ranges(i).hi);
    printf(" (");
    PRINTID_HEX(ranges(i).lo);
    printf(" - ");
    PRINTID_HEX(ranges(i).hi);
    printf(")");
    printf("\n");
  }
}
...
//const SpatialIndex &index = htm.index();
	...
	*/
	...
	char hull[100] = "J2000 6 54. 29. 54. 30. 55. 29. 55. 30.";
//	char hull[100] = "CONVEX J2000 6 54. 29. 54. 30. 55. 39. 55. 30.";
	nRanges = htm.convexHull(hull,ranges);

const ValVec<htmRange> *rlist;
SpatialDomain domain;
rlist = &htm.domain(domain);

	if(htm.err())
		cout << htm.error() << endl;
	else {
		printf("Ranges for rectangle %s\n",hull);
		printRanges(nRanges,ranges);
	}

	/* Example of convex hull */
//	char hull[100] = "J2000 6 41.4 47.9 41.2.47.9 41.0 47.5 41.4 48";
	strcpy(hull,"J2000 6 41.4 47.9 41.2 47.9 41.0 47.5 41.4 48");
	nRanges = htm.convexHull(hull,ranges);
	if(htm.err())
		cout << htm.error() << endl;
	else {
