...
//
// LN add
// Intersect returning just the number of partial and full trixels at the given
// list of depths
void
SpatialConvex::intersect(const SpatialIndex * idx,
  ...
  doIntersectNIDs(depths, nid_partial, nid_full);
}

// Intersect excluding given depths or returning multiple depths lists
void
SpatialConvex::intersect(const SpatialIndex * idx,
                         const vector<int> depths,
  ...
  doIntersectMD(depths, nid_depths);
}

//Start intersect for number of IDs at the input depths
void
SpatialConvex::doIntersectNIDs(const vector<int> depths,
                               vector<long long int> *nid_partial,
                               vector<long long int> *nid_full) {

  simplify();

  if (constraints_.length() == 0) return;

  if (depths.size() > 0) {
    vector<int> flevel;
    if (addlevel_) {
      flevel.resize(depths.size(), index_->maxlevel_);
      for (int i = 0; i < depths.size(); i++)
        flevel[i] -= depths[i];
    }
    for(uint32 i = 1; i <= 8; i++)
      triangleTestNIDs(i, depths, flevel, nid_partial, nid_full);
  }
  ...
  if (depths.size() > 1) {
    ...
    for (int i=depths.size()-1; i>0; i--) {
      for (int j=0; j < i; j++) {
        if ((*nid_full)[j] > 0) {
          int dd = (depths[i] - depths[j]);
          (*nid_full)[i] += (one << 2*dd) * (*nid_full)[j] ;
        }
      }
    }
  }
...
SpatialMarkup
SpatialConvex::triangleTestNIDs(uint64 id, vector<int> depths,
                                vector<int> flevel,
                                vector<long long int> *nid_partial,
                                vector<long long int> *nid_full)
{
  ...
  vector<int>::iterator it = find(depths.begin(), depths.end(), currdepth_);
  if (it != depths.end()) {
    int i = it - depths.begin();
    if(mark == fULL) {
      (*nid_full)[i]++;
      return mark;
    } else
  ...
  if (NC(id,0)!=0) {
    currdepth_++;
    triangleTestNIDs(NC(id,0), depths, flevel, nid_partial, nid_full);
    triangleTestNIDs(NC(id,1), depths, flevel, nid_partial, nid_full);
    triangleTestNIDs(NC(id,2), depths, flevel, nid_partial, nid_full);
    triangleTestNIDs(NC(id,3), depths, flevel, nid_partial, nid_full);
    currdepth_--;
  } else {
    if(addlevel_) {
      testPartialNIDs(addlevel_, N(id).id_, flevel, nid_partial, nid_full, V(NV(0)), V(NV(1)), V(NV(2)));
    }
  }
...
void
SpatialConvex::testPartialNIDs(size_t level, uint64 id,
                           vector<int> flevel,
                           vector<long long int> *nid_partial,
                           vector<long long int> *nid_full,
...
//cout << "Level in: " << level << endl;
  if(level--) {
    SpatialVector w0 = v1 + v2; w0.normalize();
    SpatialVector w1 = v0 + v2; w1.normalize();
    SpatialVector w2 = v1 + v0; w2.normalize();

    testSubTriangleNIDs(level, (id << 2)    , flevel, nid_partial, nid_full, v0, w2, w1);
    testSubTriangleNIDs(level, (id << 2) + 1, flevel, nid_partial, nid_full, v1, w0, w2);
    testSubTriangleNIDs(level, (id << 2) + 2, flevel, nid_partial, nid_full, v2, w1, w0);
    testSubTriangleNIDs(level, (id << 2) + 3, flevel, nid_partial, nid_full, w0, w1, w2);
  }

}

void
SpatialConvex::testSubTriangleNIDs(size_t level, uint64 id,
                           vector<int> flevel,
                           vector<long long int> *nid_partial,
                           vector<long long int> *nid_full,
                           const SpatialVector & v0,
...
//cout << "  Level sub: " << level << endl;
  SpatialMarkup mark = testNode(v0, v1, v2);

  if(mark > fULL) return;

  vector<int>::iterator it = find(flevel.begin(), flevel.end(), level);
  if (it != flevel.end()) {
    int i = it - flevel.begin();
    if(mark == fULL) {
      (*nid_full)[i]++;
    } else {
       (*nid_partial)[i]++;
       testPartialNIDs(level, id, flevel, nid_partial, nid_full, v0, v1, v2);
    }
  } else
    testPartialNIDs(level, id, flevel, nid_partial, nid_full, v0, v1, v2);
...
//End intersect for number of IDs at the input depths





//Start intersect multiple depths lists
void
SpatialConvex::doIntersectMD(const vector<int> depths,
                             vector<long long int> *nid_depths) {
...
// Number of depths is checked in SpatialDomain
  if (depths[0] > 0) {
    vector<int> flevel;
    if (addlevel_) {

//    vector<int> flevel (depths.size(), index_->maxlevel_);
      flevel.resize(depths.size(), index_->maxlevel_);
      for (int i = 0; i < depths.size(); i++)
        flevel[i] -= depths[i];
//cout << " flevel=" << flevel <<endl;
    }
    for(uint32 i = 1; i <= 8; i++)
      triangleTestMD(i, depths, flevel, nid_depths);
...
SpatialMarkup
SpatialConvex::triangleTestMD(uint64 id, vector<int> depths,
                              vector<int> flevel,
                              vector<long long int> *nid_depths)
{
  SpatialMarkup mark;
...
// Add full nodes at the given level(s)
  if(mark == fULL) {
    vector<int>::iterator it = find(depths.begin(), depths.end(), currdepth_);
    if (it != depths.end()) {
...
//else cout <<"*** NOT the right level ***"<< endl;
  ...
  if (NC(id,0)!=0) {
    currdepth_++;
    triangleTestMD(NC(id,0), depths, flevel, nid_depths);
    triangleTestMD(NC(id,1), depths, flevel, nid_depths);
    triangleTestMD(NC(id,2), depths, flevel, nid_depths);
    triangleTestMD(NC(id,3), depths, flevel, nid_depths);
    currdepth_--;
  } else {
    if(addlevel_) {

      testPartialMD(addlevel_, N(id).id_, flevel, nid_depths, V(NV(0)), V(NV(1)), V(NV(2)));
...
void
SpatialConvex::testPartialMD(size_t level, uint64 id,
                           vector<int> flevel,
                           vector<long long int> *nid_depths,
                           const SpatialVector & v0,
                           const SpatialVector & v1,
...
//cout << "Level in: " << level << endl;
  if(level--) {
    ...
    testSubTriangleMD(level, (id << 2)    , flevel, nid_depths, v0, w2, w1);
    testSubTriangleMD(level, (id << 2) + 1, flevel, nid_depths, v1, w0, w2);
    testSubTriangleMD(level, (id << 2) + 2, flevel, nid_depths, v2, w1, w0);
    testSubTriangleMD(level, (id << 2) + 3, flevel, nid_depths, w0, w1, w2);
  } else {
    if(bitresult_)
...
void
SpatialConvex::testSubTriangleMD(size_t level, uint64 id,
                           vector<int> flevel,
                           vector<long long int> *nid_depths,
                           const SpatialVector & v0,
                           const SpatialVector & v1,
...
//cout <<"*** NOT the right level ***"<< endl;
      testPartialMD(level, id, flevel, nid_depths, v0, v1, v2);
    }
...
//End intersect multiple depths lists
