...
cout<<"Spherematch2: Error: matchlength = "<< matchlength <<", minchunksize = "<< minchunksize <<endl;
  ...
  long maxmatch, jmax, i, j, k, i1, j1, k1, rachunk, decchunk, retval=1;
...
// This is required to manage multiple matches in ref catalogue
        if (jmax > 1 && sep <= matchlength) {  // keep only those within max distance
          ...
          d12.push_back(sep);
          j1++;
//cout<<"i, k, j, minsep: "<<i<<" "<<k<<" "<<j<<" "<<minsep*3600<<endl;
        }
...
//for (i=0; i<n; i++) {
//cout <<"i: "<<i<<" m2[i].index: "<<m2[i].index<<" match1[i]: "<<match1[i]<<" m2[i].id: "<<m2[i].id<<" d: "<<distance12[m2[i].index]<<endl;
//}
    ...
    for (i=1; i<n; i++) {
//cout<<"i: "<< i <<"  m2[i-1].id: "<<m2[i-1].id<<"  m2[i].id: "<<m2[i].id<<endl;
...
// Duplicated reference objects
      ...
      if (m2[i].id == -1) continue;
      if (m2[i-1].id == m2[i].id || m2[i].id == dup_id) {
        dup_id = m2[i].id;
        ndup++;
//cout<<"i: "<<i<<" ndup="<<ndup<<" dup_id="<<dup_id<<"  m2[i-1].id: "<<m2[i-1].id<<"  m2[i].id: "<<m2[i].id<<" match1[i]: "<<match1[i]<<endl;
cout<<"Spherematch2: duplicated reference entry: m2[i].id="<< m2[i].id <<"  "<<ra2[m2[i].id]<<" "<<dec2[m2[i].id]
    <<" d1="<< distance12[m2[i-1].index]*3600 <<" d2="<< distance12[m2[i].index]*3600 <<endl;

        if (distance12[m2[i].index] <= distance12[m2[i-1].index] && distance12[m2[i].index] < dmin)  // following object is closer
        {
...
*/
          ...
          if (distance12[m2[i-1].index] < dmin) dmin = distance12[m2[i-1].index];
/* Example:
   id1                             id2
...
//cout<<"dmin="<<dmin*3600<<" distance12[m2[i1].index]="<< distance12[m2[i1].index]*3600<<"  m2[i-1].id: "<<m2[i-1].id<<"  m2[i].id: "<<m2[i].id<<endl;
      }  // end if
      else dmin = 361;
