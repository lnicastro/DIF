...
// This is required to manage multiple matches in ref catalogue
        if (jmax > 1 && sep <= matchlength) {  // keep only those within max distance
          ...
          d12.push_back(sep);
          j1++;
cout<<"***jmax, i, k, j, minsep: "<<jmax<<"  "<<i<<" "<<k<<" "<<j<<" "<<minsep*3600<<endl;
        }
...
cout<<"id1["<<j<<"]="<<id1[j]<<"  match1["<<m2[ij].index<<"]="<<match1[m2[ij].index]
           <<" id2["<<j<<"]="<<id2[j]<<" m2["<<i<<"].id="<<m2[i].id<<endl;
            if (id1[j] == match1[m2[ij].index] && id2[j] != m2[i].id) {

cout<<"Swapping ids: Old refcat id="<<match2[m2[ij].index]<<" d="<<distance12[m2[ij].index]*3600;
              match2[m2[ij].index] = id2[j];
              distance12[m2[ij].index] = d12[j];
cout<<" New refcat id="<<id2[j]<<" d="<<d12[j]*3600<<endl;
              break;
            }
          }
...
cout<<"Previous duplicated object is closer.\n";
          ...
          for (j=0; j<nmul; j++) {
            ...
            if (id1[j] == match1[m2[i].index] && id2[j] != m2[i].id) {
// 71 && 70 
cout<<"Swapping ids: Old refcat id="<<match2[m2[i].index]<<" d="<<distance12[m2[i].index]*3600;
              match2[m2[i].index] = id2[j];
// 71 -> 70
              distance12[m2[i].index] = d12[j];
cout<<" New refcat id="<<id2[j]<<" d="<<d12[j]*3600<<endl;
              break;
            }
          }
